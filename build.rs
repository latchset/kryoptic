...
fn build_ossl() {
    ...
    let (libpath, bldargs) = {
        ...
        let buildargs = ["--debug", "enable-fips", "no-mdc2", "no-ec2m", "no-sm2", "no-sm4", "-DREDHAT_FIPS_VERSION=\\\"0.0.1-test\\\""];
    ...
    let (libpath, bldargs) = {
        let libcrypto = format!("{}/libcrypto.a", openssl_path.to_string_lossy());
