...
impl bindgen::callbacks::ParseCallbacks for OsslCallbacks {
    fn int_macro(
        ...
        if name == "OPENSSL_VERSION_NUMBER" {
            // This is exposed to the ossl as DEP_OPENSSL_VERSION_NUMBER (DEP_<LINKS>_<KEY>)
            // in the same way openssl-sys does
            println!("cargo:version_number={value:x}");
        }
...
fn main() {
    ...
    if cfg!(all(feature = "dynamic", feature = "fips")) {
        panic!("You can't combine dynamic and fips features.")
    }
