...
impl AesOperation {
    ...
    /// Decryption Initialization helper
    ///
    /// Sets up all the required context or parameters setting to direct
    /// the underlying OpenSSL crypto library, based on the configured
    /// mechanism and all the parameters stored on the object.
...
impl Encryption for AesOperation {
    ...
    /// Provides the expect buffer size for the provided input plaintext length
...
impl Decryption for AesOperation {
    ...
    /// Provides the expect buffer size for the provided input ciphertext length
    /// based on the selected mode of operation.
    ///
    /// May return different values depending on the internal status and the
    /// mode of operation.
...
impl MsgEncryption for AesOperation {
    ...
    /// Feed the next plaintext buffer to be encrypted
    ///
    ...
    /// Feed the final plaintext buffer to be encrypted
    ///
    /// Returns output data in the provided cipher buffer
...
impl MsgDecryption for AesOperation {
    ...
    /// Feed the next ciphertext buffer to be decrypted
    ///
    ...
    /// Feed the final ciphertext buffer to be decrypted
    ///
    /// Returns output data in the provided plain buffer
...
impl AesCmacOperation {
    ...
    /// Feeds the next data buffer into the CMAC computation
...
impl AesMacOperation {
    ...
    /// Feeds the next data buffer into the MAC computation
    fn update(&mut self, data: &[u8]) -> Result<()> {
        if self.finalized {
            return Err(CKR_OPERATION_NOT_INITIALIZED)?;
        }
