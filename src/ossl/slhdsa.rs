...
//! This module implements the SLH-DSA signature mechanisms as defined in FIPS
//! 205 using OpenSSL (3.5+) EVP_PKEY functions. It handles key generation,
//! signing, verification, and parameter/context handling for different SLH-DSA
//! variants.
...
/// Maximum expected DER length for a hash OID used in Hash-SLH-DSA M'
/// construction. (17 bytes is probably sufficient, but we put some
/// headroom here) */
const MAX_OID_DER_LEN: usize = 20;
/// Maximum expected hash output length used in Hash-SLH-DSA M' construction.
const MAX_HASH_LEN: usize = 64;

/// Maps a PKCS#11 SLH-DSA parameter set type (`CK_SLH_DSA_PARAMETER_SET_TYPE`)
...
/// Converts a PKCS#11 SLH-DSA key `Object` into `EvpPkey`.
///
/// Extracts the parameter set (`CKA_PARAMETER_SET`) to determine the algorithm
/// name. Extracts key components (`CKA_VALUE` for public/private key) based on
/// the object `class` and populates an `SlhDsaData` structure.
...
/// Holds parsed parameters specific to an SLH-DSA operation instance.
#[derive(Debug)]
struct SlhDsaParams {
    /// The SLH-DSA parameter set (e.g., CKP_SLH_DSA_SHA2_128S).
    param_set: CK_SLH_DSA_PARAMETER_SET_TYPE,
    /// The hedging variant requested (e.g., CKH_HEDGE_PREFERRED).
    hedge: CK_HEDGE_TYPE,
    /// Optional context string.
    context: Option<Vec<u8>>,
    /// Hash mechanism for CKM_HASH_SLH_DSA.
...
impl SlhDsaParams {
    /// Creates a new `SlhDsaParams` instance by parsing mechanism parameters.
    pub fn new(
        mech: &CK_MECHANISM,
        param_set: CK_SLH_DSA_PARAMETER_SET_TYPE,
    ) -> Result<SlhDsaParams> {
...
/// Represents an active SLH-DSA signing or verification operation.
#[derive(Debug)]
pub struct SlhDsaOperation {
    /// The specific SLH-DSA mechanism being used.
    mech: CK_MECHANISM_TYPE,
    /// Flag indicating if the operation has been finalized.
    finalized: bool,
    /// Flag indicating if the operation is in progress (update called).
    in_use: bool,
    /// Parsed SLH-DSA parameters for this operation instance.
    params: SlhDsaParams,
    /// The ossl signature context.
    sigctx: OsslSignature,
    /// Size of the hash for Hash-SLH-DSA variants.
    hashsize: usize,
    /// Optional hasher instance for Hash-SLH-DSA variants.
    hasher: Option<Box<dyn Digest>>,
    /// Stored signature for VerifySignature operations for HashSLH-DSA
...
impl SlhDsaOperation {
    /// Creates a new SLH-DSA sign or verify operation context.
    ///
    ...
    /// for SLH-DSA and sets up internal hashing if a Hash-SLH-DSA variant is
    /// used.
    pub fn sigver_new(
        mech: &CK_MECHANISM,
        key: &Object,
        ...
        /* OpenSSL 3.5.0 does not offer HashSLH-DSA, so we'll
    ...
    fn set_signature(&mut self, signature: &[u8]) -> Result<()> {
         ...
         *   HashSLH-DSA at all, so we always store the signature
    ...
    /// Sets up the internal hasher for Hash-SLH-DSA variants based on the
    /// mechanism type.
    ...
    /// Compute M' for Hash-SLH-DSA
    ///
    /// For Hash-SLH-DSA the encoding is:
    /// M' = 01 || ctx_len || ctx || OID || Hash(msg)
    /// See FIPS-205 Algorithm 23 Step 24
    /// (same as the ML-DSA one)
    ...
    /// Internal helper to update the internal hasher for Hash-SLH-DSA variants.
    ...
    /// Internal helper to finalize the internal hasher for Hash-SLH-DSA
    /// variants.
    ...
    fn verify_internal(
        ...
        match self.mech {
            CKM_HASH_SLH_DSA => {
                ...
                /* For CKM_HASH_SLH_DSA the data is the hash! */
    ...
    fn verify_int_update(&mut self, data: &[u8]) -> Result<()> {
        ...
        match self.mech {
            ...
            CKM_HASH_SLH_DSA => {
                /* CKM_HASH_SLH_DSA is single-part only */
                self.finalized = true;
                return Err(CKR_OPERATION_NOT_INITIALIZED)?;
            }
            CKM_HASH_SLH_DSA_SHA224
...
/// Populates the public (`CKA_VALUE`) and private (`CKA_VALUE`)
