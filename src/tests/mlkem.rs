...
#[test]
#[parallel]
fn test_mlkem_public_key_info() {
    let mut testtokn =
        TestToken::initialized("test_mlkem_public_key_info", None);
    let session = testtokn.get_session(true);
...
fn test_mlkem_template_attributes() {
    ...
    // Create keys with CKA_ENCAPSULATE_TEMPLATE on public key requiring CKA_ENCRYPT = true
    // and CKA_DECAPSULATE_TEMPLATE on private key requiring CKA_DECRYPT = true
    ...
    let enc_tmpl = unsafe {
        std::slice::from_raw_parts(
            inner_enc_tmpl.as_ptr() as *const u8,
            std::mem::size_of_val(&inner_enc_tmpl),
        )
    };
    ...
    let dec_tmpl = unsafe {
        std::slice::from_raw_parts(
            inner_dec_tmpl.as_ptr() as *const u8,
            std::mem::size_of_val(&inner_dec_tmpl),
        )
    };
    ...
    let (pub_handle, priv_handle) = ret_or_panic!(generate_key_pair(
        ...
        &[(CKA_ENCAPSULATE_TEMPLATE, enc_tmpl)],
        &[(CKA_ENCAPSULATE, true)],
        &[(CKA_CLASS, CKO_PRIVATE_KEY), (CKA_KEY_TYPE, CKK_ML_KEM),],
        &[(CKA_DECAPSULATE_TEMPLATE, dec_tmpl)],
        &[(CKA_DECAPSULATE, true)],
    ));
