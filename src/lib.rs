...
impl GlobalConfig {
    ...
    fn default_config() -> GlobalConfig {
        ...
        let conf = match Config::from_file(&filename) {
            Ok(n) => n,
            Err(_) => {
                let mut gc = Self::empty_config();
                ...
                if filename.ends_with(".sql") {
                    match storage::name_to_type(&filename) {
                        Ok(t) => {
                            let mut slot = config::Slot::new();
                            slot.dbtype = Some(t.to_string());
                            slot.dbpath = Some(filename);
                            /* if this fails there will be no slots defined */
                            let _ = gc.conf.add_slot(slot);
                        }
                        Err(_) => (),
                    }
                }
    ...
    fn slot_from_init_args(&mut self, args: &str) -> Result<()> {
        let mut slot = config::Slot::new();

        let arguments: Vec<&str> = args.split(',').collect();
        for arg in &arguments {
            let comps: Vec<&str> = arg.splitn(2, '=').collect();
            if comps.len() != 2 {
                return Err(CKR_ARGUMENTS_BAD)?;
            }
            match comps[0] {
                "db" => {
                    let dbv: Vec<&str> = comps[1].splitn(2, ':').collect();
                    if dbv.len() == 1 {
                        match storage::name_to_type(dbv[0]) {
                            Ok(t) => {
                                slot.dbtype = Some(t.to_string());
                                slot.dbpath = Some(dbv[0].to_string());
                            }
                            Err(_) => {
                                if dbv[0] == "memory" {
                                    slot.dbtype =
                                        Some(storage::MEMORYDB.to_string());
                                } else {
                                    return Err(CKR_ARGUMENTS_BAD)?;
                                }
                            }
                        ...
                        match dbv[0] {
                            "sqlite" => {
                                slot.dbtype =
                                    Some(storage::SQLITEDB.to_string());
                                slot.dbpath = Some(dbv[1].to_string());
                            }
                            "json" => {
                                slot.dbtype = Some(storage::JSONDB.to_string());
                                slot.dbpath = Some(dbv[1].to_string());
                            }
                            "memory" => {
                                slot.dbtype = Some(dbv[0].to_string());
                            }
                ...
                "slot" => {
                    slot.slot = match comps[1].parse::<u32>() {
                        Ok(n) => n,
        ...
        for s in &self.conf.slots {
            if s.slot == slot.slot {
                ...
                /* already present so return */
                return Ok(());
            }
        }

        self.conf.add_slot(slot)
    }
}
...
extern "C" fn fn_initialize(_init_args: CK_VOID_PTR) -> CK_RV {
    ...
    if !_init_args.is_null() {
        ...
        if !args.pReserved.is_null() {
            ...
            res_or_ret!(gconf.slot_from_init_args(init_arg));
        }
    }
