...
//! Module containing data to check Well Known Groups
//! We need to duplicate data here, because OpenSSL does not give us
...
/// RFC 7719 Appendix A.1 ffdhe3072 prime
/// p = 2^3072 - 2^3008 + {[2^2942 * e] + 2625351} * 2^64 - 1
static FFDHE3072_P: [u8; 384] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xAD, 0xF8, 0x54, 0x58,
    0xA2, 0xBB, 0x4A, 0x9A, 0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1,
    0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, 0xA9, 0xE1, 0x36, 0x41,
    0x14, 0x64, 0x33, 0xFB, 0xCC, 0x93, 0x9D, 0xCE, 0x24, 0x9B, 0x3E, 0xF9,
...
/// RFC 7719 Appendix A.1 ffdhe4096 prime
/// p = 2^4096 - 2^4032 + {[2^3966 * e] + 5736041} * 2^64 - 1
static FFDHE4096_P: [u8; 512] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xAD, 0xF8, 0x54, 0x58,
    0xA2, 0xBB, 0x4A, 0x9A, 0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1,
    0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, 0xA9, 0xE1, 0x36, 0x41,
    0x14, 0x64, 0x33, 0xFB, 0xCC, 0x93, 0x9D, 0xCE, 0x24, 0x9B, 0x3E, 0xF9,
...
/// RFC 7719 Appendix A.1 ffdhe6144 prime
/// p = 2^6144 - 2^6080 + {[2^6014 * e] + 15705020} * 2^64 - 1
static FFDHE6144_P: [u8; 768] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xAD, 0xF8, 0x54, 0x58,
    0xA2, 0xBB, 0x4A, 0x9A, 0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1,
    0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, 0xA9, 0xE1, 0x36, 0x41,
    0x14, 0x64, 0x33, 0xFB, 0xCC, 0x93, 0x9D, 0xCE, 0x24, 0x9B, 0x3E, 0xF9,
...
/// RFC 7719 Appendix A.1 ffdhe8192 prime
/// p = 2^8192 - 2^8128 + {[2^8062 * e] + 10965728} * 2^64 - 1
static FFDHE8192_P: [u8; 1024] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xAD, 0xF8, 0x54, 0x58,
    0xA2, 0xBB, 0x4A, 0x9A, 0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1,
    0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, 0xA9, 0xE1, 0x36, 0x41,
    0x14, 0x64, 0x33, 0xFB, 0xCC, 0x93, 0x9D, 0xCE, 0x24, 0x9B, 0x3E, 0xF9,
...
/// RFC 3526 - 6: 6144-bit MODP Group
/// p = 2^6144 - 2^6080 - 1 + 2^64 * { [2^6014 pi] + 929484 }
static MODP6144_P: [u8; 768] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC9, 0x0F, 0xDA, 0xA2,
    0x21, 0x68, 0xC2, 0x34, 0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1,
    0x29, 0x02, 0x4E, 0x08, 0x8A, 0x67, 0xCC, 0x74, 0x02, 0x0B, 0xBE, 0xA6,
    0x3B, 0x13, 0x9B, 0x22, 0x51, 0x4A, 0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD,
...
/// RFC 3526 - 7: 8192-bit MODP Group
/// p = 2^8192 - 2^8128 - 1 + 2^64 * { [2^8062 pi] + 4743158 }
static MODP8192_P: [u8; 1024] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC9, 0x0F, 0xDA, 0xA2,
    0x21, 0x68, 0xC2, 0x34, 0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1,
    0x29, 0x02, 0x4E, 0x08, 0x8A, 0x67, 0xCC, 0x74, 0x02, 0x0B, 0xBE, 0xA6,
    0x3B, 0x13, 0x9B, 0x22, 0x51, 0x4A, 0x08, 0x79, 0x8E, 0x34, 0x04, 0xDD,
...
static FFDHE_NAMED_GROUPS: [FFDHNamedGroup; 10] = [
    FFDHNamedGroup {
        id: DHGroupName::FFDHE2048,
        prime: &FFDHE2048_P,
        keylen: 225,
        base: &GENERATOR2,
        subgroup: &FFDHE2048_Q,
    },
    FFDHNamedGroup {
        id: DHGroupName::FFDHE3072,
        prime: &FFDHE3072_P,
        keylen: 275,
        base: &GENERATOR2,
        subgroup: &FFDHE3072_Q,
    },
    FFDHNamedGroup {
        id: DHGroupName::FFDHE4096,
        prime: &FFDHE4096_P,
        keylen: 325,
        base: &GENERATOR2,
        subgroup: &FFDHE4096_Q,
    },
    FFDHNamedGroup {
        id: DHGroupName::FFDHE6144,
        prime: &FFDHE6144_P,
        keylen: 375,
        base: &GENERATOR2,
        subgroup: &FFDHE6144_Q,
    },
    FFDHNamedGroup {
        id: DHGroupName::FFDHE8192,
        prime: &FFDHE8192_P,
        keylen: 400,
        base: &GENERATOR2,
        subgroup: &FFDHE8192_Q,
    },
    FFDHNamedGroup {
        id: DHGroupName::MODP2048,
        prime: &MODP2048_P,
        keylen: 225,
        base: &GENERATOR2,
        subgroup: &MODP2048_Q,
...
pub fn get_group_name(obj: &Object) -> Result<DHGroupName> {
    let prime = match obj.get_attr_as_bytes(CKA_PRIME) {
        Ok(p) => p.clone(),
        Err(_) => return Err(CKR_ATTRIBUTE_VALUE_INVALID)?,
    };

    let base = match obj.get_attr_as_bytes(CKA_BASE) {
        Ok(b) => b.clone(),
        Err(_) => return Err(CKR_ATTRIBUTE_VALUE_INVALID)?,
    };
...
pub fn group_values(
    ...
    for grp in &FFDHE_NAMED_GROUPS {
        if grp.id == group {
            return Ok((grp.prime, grp.base, grp.subgroup));
        }
    }
