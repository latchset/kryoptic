...
impl RsaData {
    ...
    fn to_params<'a>(
        ...
        let mut pkey_class: FfiPkeyClassType = EVP_PKEY_PUBLIC_KEY;
        params_builder
...
impl EvpPkey {
    ...
    #[cfg(all(not(feature = "openssl-sys"), not(feature = "fips")))]
    ...
    /// Openssl-sys does not expose i2d_PUBKEY, so we'll have to
    /// unroll our own
    #[cfg(feature = "openssl-sys")]
    pub fn spki_der(&self) -> Result<Vec<u8>, Error> {
        unsafe {
            let ctx = OSSL_ENCODER_CTX_new_for_pkey(
                self.ptr,
                EVP_PKEY_PUBLIC_KEY as c_int,
                c"DER".as_ptr(),
                c"SubjectPublicKeyInfo".as_ptr(),
                std::ptr::null(),
            );
            if ctx.is_null() {
                trace_ossl!("OSSL_ENCODER_CTX_new_for_pkey()");
                return Err(Error::new(ErrorKind::OsslError));
            }

            let mut pdata: *mut u8 = std::ptr::null_mut();
            let mut pdata_len: usize = 0;

            if OSSL_ENCODER_CTX_get_num_encoders(ctx) == 0
                || OSSL_ENCODER_to_data(ctx, &mut pdata, &mut pdata_len) == 0
                || pdata.is_null()
            {
                trace_ossl!("OSSL_ENCODER_to_data()");
                return Err(Error::new(ErrorKind::OsslError));
            }

            let slice = std::slice::from_raw_parts(pdata, pdata_len);
            let ret_vec = slice.to_vec();

            OPENSSL_free(pdata as *mut c_void);
