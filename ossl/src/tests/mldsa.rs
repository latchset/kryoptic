...
use crate::pkey::{EvpPkey, EvpPkeyType, PkeyData};
use crate::signature::{
    mldsa_params, mldsa_with_mu, MuGenerator, OsslSignature, SigAlg, SigOp,
};
use crate::tests::test_ossl_context;

#[test]
fn test_mldsa_vector_with_mu() {
    let libctx = test_ossl_context();

    // ML-DSA-44 Test Vector from NIST ACVP (pure ML-DSA with context)
    let msg = hex::decode(
    ...
    .expect("failed to decode test public key");
    ...
    .expect("failed to decode test context");
    ...
    .expect("failed to decode test signature");

    let key = EvpPkey::import(
        libctx,
        EvpPkeyType::Mldsa44,
        PkeyData::Mlkey(crate::pkey::MlkeyData {
            pubkey: Some(pk.clone()),
            prikey: None,
            seed: None,
        }),
    )
    .expect("Failed to import ML-DSA-44 public key");

    // Compute MU correctly
    let mut mu = vec![0u8; 64];
    let mut mu_gen = MuGenerator::new(libctx, &key, Some(&ctx))
        .expect("Failed to create mu generator");
    mu_gen.update(&msg[0..50]).unwrap();
    mu_gen.finalize(Some(&msg[50..]), &mut mu).unwrap();

    // Verify using MU
    let verify_params =
        mldsa_with_mu(Some(&ctx), false).expect("Failed to build params");
    let mut ver_ctx = OsslSignature::new(
        libctx,
        SigOp::Verify,
        SigAlg::Mldsa44,
        &key,
        verify_params.as_ref(),
    )
    .expect("Failed to create verify context");

    ver_ctx
        .verify(&mu, Some(&sig))
        .expect("Failed to verify known test vector with MU");

    // Test reinit
    let mut mu2 = vec![0u8; 64];
    mu_gen.reinit().expect("Failed to reinit mu generator");
    mu_gen.update(&msg[0..50]).unwrap();
    mu_gen.finalize(Some(&msg[50..]), &mut mu2).unwrap();

    assert_eq!(mu, mu2, "reinit generated different mu for same message");

    // Verification should fail with wrong MU
    let bad_mu = vec![0x43; 64];
    assert!(
        ver_ctx.verify(&bad_mu, Some(&sig)).is_err(),
        "Verification should fail with wrong MU"
    );
}

#[test]
fn test_mldsa_sign_with_mu() {
    let libctx = test_ossl_context();

    let key = EvpPkey::generate(libctx, EvpPkeyType::Mldsa44)
        .expect("Failed to generate ML-DSA-44 key");

    let msg = b"Testing ML-DSA signature with MU generator and regular verify";
    let ctx = b"sample_context_string".to_vec();

    // Compute MU
    let mut mu = vec![0u8; 64];
    let mut mu_gen = MuGenerator::new(libctx, &key, Some(&ctx))
        .expect("Failed to create mu generator");
    mu_gen.update(msg).expect("Failed to update mu generator");
    mu_gen
        .finalize(None, &mut mu)
        .expect("Failed to finalize mu generator");

    // Generate signature using MU
    let sign_params =
        mldsa_with_mu(Some(&ctx), false).expect("Failed to build sign params");
    let mut sign_ctx = OsslSignature::new(
        libctx,
        SigOp::Sign,
        SigAlg::Mldsa44,
        &key,
    ...
    let mut sig = vec![0u8; key.get_size().expect("Failed to get key size")];
    let siglen = sign_ctx
        .sign(&mu, Some(&mut sig))
        .expect("Failed to sign with MU");
    sig.truncate(siglen);

    // Verify using regular verify operation (w/o custom MU calculation)
    let verify_params = mldsa_params(false, Some(&ctx), false)
        .expect("Failed to build verify params");
    let mut ver_ctx = OsslSignature::new(
        libctx,
        SigOp::Verify,
        SigAlg::Mldsa44,
    ...
    ver_ctx
        .verify(msg, Some(&sig))
        .expect("Failed to verify signature with regular verify operation");

    // Verification should fail with altered message
    let bad_msg = b"Tampered message";
    assert!(
        ver_ctx.verify(bad_msg, Some(&sig)).is_err(),
        "Verification should fail with altered message"
    );
}
